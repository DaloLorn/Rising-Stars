	...
	void updateFleetStrength(Object& obj) {
		...
		for(uint i = 0, cnt = supports.length; i < cnt; ++i) {
			...
			if(ship !is null) {
				...
				hp += ((bp.currentHP * bp.hpFactor + (getRepairFor(ship)/3.0*pow(max(log10(getRepairFor(ship)/3.0),0.0),2))) * (1.0+log10(bp.design.size)*0.1) + ((1.0+max(log10(bp.getEfficiencySum(SV_ShieldRegen) / (1.0 - ship.mitigation))*1.5, 1.0)) * (ship.Shield / (1.0 - ship.mitigation)))) * ShieldBehaviorMod;

				dps += ship.DPS * bp.shipEffectiveness;
				// Fleet Calc
				maxHP += ((bp.design.totalHP + (getRepairFor(bp.design, obj.owner)/3.0*pow(max(log10(getRepairFor(bp.design, obj.owner)/3.0),0.0),2))) * (1.0+log10(bp.design.size)*0.1) + ((1.0+max(log10(bp.design.total(SV_ShieldRegen) / (1.0 - ship.mitigation))*1.5, 1.0)) * ship.MaxShield)) * ShieldBehaviorMod;
				maxDPS += ship.MaxDPS;
				// END NON-MIT CODE
			}
		}
