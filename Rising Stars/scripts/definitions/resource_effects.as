...
#section server
	void onTick(Object& obj, Resource@ r, double time, any@ data) const override {
...
#section server
	...
	void load(Resource@ r, any@ data, SaveFile& file) const override {
		int amt = 0;
		file >> amt;
		data.store(amt);
	}
	
	void save(Resource@ r, any@ data, SaveFile& file) const override {
		int amt = 0;
		data.retrieve(amt);
		file << amt;
	}

	void onAdd(Object& obj, Resource@ r, any@ data) const override {
		int amt = 0;
		data.store(amt);
	}

	void onRemove(Object& obj, Resource@ r, any@ data) const override {
		int prev = 0;
		data.retrieve(prev);
		if(prev != 0) {
...
#section server
	...
	void onOwnerChange(Object& obj, Resource@ r, Empire@ prevOwner, Empire@ newOwner, any@ data) const override {
...
#section server
	...
	void onGenerate(Object& obj, Resource@ r, any@ data) const {
		int64 id = -1;
		data.store(id);
	}

	void onDestroy(Object& obj, Resource@ r, any@ data) const {
		int64 id = -1;
		data.retrieve(id);
		if(id != -1) {
	...
	void nativeTick(Object& obj, Resource@ r, double time, any@ data) const override {
		int64 id = -1;
		data.retrieve(id);
	...
	void nativeSave(Resource@ r, any@ data, SaveFile& file) const override {
		int64 id = 0;
		data.retrieve(id);
		file << id;
	}

	void nativeLoad(Resource@ r, any@ data, SaveFile& file) const override {
		if(file >= SV_0007) {
			int64 id = 0;
			file >> id;
			data.store(id);
...
#section server
	void nativeTick(Object& obj, Resource@ native, double time, any@ data) const {
...
#section server
	...
	void nativeTick(Object& obj, Resource@ native, double time, any@ data) const override {
		if(hook !is null)
			hook.tick(obj, data, time);
	}

	void onDestroy(Object& obj, Resource@ native, any@ data) const override {
		if(hook !is null)
			hook.disable(obj, data);
	}

	void nativeSave(Resource@ native, any@ data, SaveFile& file) const override {
		if(hook !is null)
			hook.save(data, file);
	}

	void nativeLoad(Resource@ native, any@ data, SaveFile& file) const override {
		if(hook !is null)
			hook.load(data, file);
	}
#section all
