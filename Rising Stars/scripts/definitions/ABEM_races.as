...
class SafeAutoOrderOnAutoColonizeTarget : AbilityHook {
	Document doc("Automatically order this ability on planets taken from the auto-colonization list, when the object is otherwise idle. Can be used safely with abilities which do not physically colonize a planet.");
	// A hack to make sure we're thinking about the same target the ability is supposed to colonize.
	Argument objTarg(TT_Object, doc="The target being colonized.");
...
#section server
	void changeTarget(Ability@ abl, any@ data, uint index, Target@ oldTarget, Target@ newTarget) const {
		...
		data.retrieve(@info);
		if(info.cur !is null) {
			// Were we targeting an autocolonize target and can still target it, OR are we not retargeting to our autocolonize target?
			if(((info.cur is oldTarget.obj && abl.isValidTarget(index, oldTarget)) || (info.cur !is newTarget.obj && info.cur !is oldTarget.obj))) {
				// We're clearly not autocolonizing at this point.
				if(info.cur.owner !is abl.emp && !info.cur.owner.valid)
					abl.emp.pushAutoColonizeTarget(info.cur);
				@info.cur = null;
			}
		}
		@info.targ = newTarget;
	...
	void destroy(Ability@ abl, any@ data) const override {
		...
		data.retrieve(@info);
		if(info.cur !is null) {
			if((info.targ is null || (info.cur is info.targ.obj && abl.isValidTarget(uint(objTarg.integer), info.targ)) || info.cur !is info.targ.obj) && info.cur.owner !is abl.emp && !info.cur.owner.valid)
				abl.emp.pushAutoColonizeTarget(info.cur);
			@info.cur = null;
		}
	}
